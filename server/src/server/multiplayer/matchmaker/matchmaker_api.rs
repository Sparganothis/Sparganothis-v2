...
use crate::server::{db::guest_login::{deserialize_base64, serialize_base64}, multiplayer::matchmaker::matchmaker_basic::{get_lock_values, increment_redis_counter, set_lock}};

use super::matchmaker_basic::run_basic_multiplayer_matchmaker;


pub async fn run_multiplayer_matchmaker_1(_from: NodeIdentity, arg: GameMatchType) -> anyhow::Result<Vec<NodeIdentity>> {
    ...
    let mut _basic = run_basic_multiplayer_matchmaker(identity_str, &game_type_string, n).await?;
    _basic.sort();
    let match_identities = _basic.iter().map(|x| deserialize_base64(x.to_string())).collect::<anyhow::Result<Vec<NodeIdentity>>>()?;
    return Ok(match_identities);
}

pub async fn run_multiplayer_matchmaker_2(_from: NodeIdentity, (arg, match_identities): (GameMatchType, Vec<NodeIdentity>)) -> anyhow::Result<GameMatch<NodeIdentity>> {
    let combo_key = match_identities.iter().map(|x| serialize_base64(x)).collect::<anyhow::Result<Vec<_>>>()?;
    ...
    tracing::debug!("Matchmaking for {} finished - {}", _from.nickname(), val.match_id);
    Ok(val)
}