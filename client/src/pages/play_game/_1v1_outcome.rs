...
use protocol::{
    server_chat_api::api_declarations::GetLastGameStatesForMatch,
...
pub fn Game1v1OutcomePage(
    ...
    let last_moves = use_resource(move || {
        ...
        async move {
            let last_states = api
                .call_method::<GetLastGameStatesForMatch>(game_match.clone())
                .await;
            match last_states {
                Err(e) => {
            ...
            article {
                ...
                if let Some(e) = error_str.read().clone() {
                    ...
                    div {
                        ...
                        if *is_loaded.read() {
                            GameDisplay {
                                game_state: last_moves.read()[0].0.clone(),
                                node_id: last_moves.read()[0].1.clone(),
                            }
                        } else {
                            "loading..."
                        }
                    ...
                    div {
                        ...
                        if  *is_loaded.read() {
                            GameDisplay {
                                game_state: last_moves.read()[1].0.clone(),
                                node_id: last_moves.read()[1].1.clone(),
                            }
                        } else {
                            "loading..."
                        }
