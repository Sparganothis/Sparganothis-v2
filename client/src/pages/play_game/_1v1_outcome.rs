...
use protocol::{server_chat_api::api_declarations::GetLastGameStatesForMatch, user_identity::NodeIdentity};

use crate::{comp::{game_display::GameDisplay, multiplayer::_1v1::Outcome1v1WindowTitle}, network::NetworkState, route::UrlParam};
...
pub fn Game1v1OutcomePage(
    ...
    let Some(api) = use_context::<NetworkState>().client_api_manager.read().clone() else {
        return rsx!{
            "loading..."
        }
    };
    ...
    let last_moves = use_resource(move || {
        ...
        let api = api.clone();
        async move {
            let last_states = api.call_method::<GetLastGameStatesForMatch>(game_match).await;
            match last_states { 
                Err(e) => {
    ...
    let last_moves = use_memo(move || last_moves.read().clone().unwrap_or(vec![]));
