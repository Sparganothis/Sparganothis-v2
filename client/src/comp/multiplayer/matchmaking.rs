...
use game::{api::game_match::{GameMatch, GameMatchType}, timestamp::get_timestamp_now_ms};
use n0_future::StreamExt;
use protocol::{
    global_matchmaker::GlobalMatchmaker, server_chat_api::{api_declarations::{RunMultiplayerMatchmakerPhase1, RunMultiplayerMatchmakerPhase2, SendNewMatch}, client_api_manager::ClientApiManager}, user_identity::NodeIdentity
};
...
async fn run_matchmaker_once(api: ClientApiManager, game_type: GameMatchType) -> anyhow::Result<GameMatch<NodeIdentity>> {

     let phase1 = api.call_method::<RunMultiplayerMatchmakerPhase1>(game_type.clone()).await?;
     let phase2 = api.call_method::<RunMultiplayerMatchmakerPhase2>((game_type, phase1)).await?;
    Ok(phase2)
}

async fn run_matchmaker(mm: GlobalMatchmaker, api: ClientApiManager, game_type: GameMatchType) -> anyhow::Result<GameMatch<NodeIdentity>> {
...
pub fn MatchmakingWindow(
    ...
    let coro = use_coroutine(move |mut _r| {
        ...
        while let Some(_x) = _r.next().await {
            let mut game_fut = run_matchmaker(mm.clone(), api.clone(), user_match_type.read().clone())
            .fuse()
            .boxed();
            timer_w.set(0);
            let game = loop {
