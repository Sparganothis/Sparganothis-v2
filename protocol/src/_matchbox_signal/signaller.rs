...
use crate::{
    _matchbox_signal::direct_message::{send_direct_message, DirectMessageProtocol, DIRECT_MESSAGE_ALPN},
    get_timestamp,
};
...
pub struct IrohGossipSignallerBuilder {
    ...
    pub(crate) direct_message_recv: async_broadcast::InactiveReceiver<(PublicKey, PeerEvent)>,
}
                ...
                gossip_msg = gossip_recv.next().fuse() => {
                    ...
                    match gossip_msg {
                        ...
                        Event::Gossip(GossipEvent::Received(Message { content: gossip_msg, ..})) => {
                            let GossipMessage{iroh_id, matchbox_id, ..} = postcard::from_bytes(&gossip_msg)?;
                            let now = Instant::now();
